...
export class CardTypeDetector {
	 ...
	 * 计算最终题型和置信度
	 */
	private static calculateFinalType(
		features: DetectedFeature[],
		card: Card
	): { cardType: UnifiedCardType; confidence: number } {
		// 计算各题型的得分
		...
		logger.debug(
			`[CardTypeDetector] 检测结果: ${cardType}, 置信度: ${confidence.toFixed(2)}, 得分:`,
			scores
		);
	...
	private static calculateAllTypeScores(card: Card, content: string): Record<string, number> {
		...
		scores[UnifiedCardType.MULTIPLE_CHOICE] =
			card.fields && isMultipleChoiceCard(card.fields) ? 0.8 : 0;
	...
	static getFeatureDescription(feature: DetectedFeature): string {
		...
		return `${typeDescriptions[feature.type] || feature.type}: ${feature.pattern} (置信度: ${(
