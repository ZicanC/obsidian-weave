...
export class ExtensiblePreviewManager implements ExtensionRegistry {
	...
	async detectCardType(card: Card): Promise<CardTypeDetectionResult | null> {
		const detectors = Array.from(this.detectors.values()).sort((a, b) => b.priority - a.priority); // 按优先级排序
		...
		for (const detector of detectors) {
			try {
				...
				if (result.matches && result.confidence > 0.7) {
					logger.debug(
						`[ExtensiblePreviewManager] 检测到题型: ${result.cardTypeId} (置信度: ${result.confidence})`
					);
					return result;
				}
			} catch (error) {
	 ...
	 * 生成扩展预览
	 */
	async generateExtensiblePreview(
		card: Card,
		cardTypeId: string,
		options: PreviewOptions
	): Promise<PreviewData | null> {
		const generator = Array.from(this.generators.values()).find((g) =>
			g.supportedTypes.includes(cardTypeId)
		);
	 ...
	 * 渲染扩展预览
	 */
	async renderExtensiblePreview(
		previewData: PreviewData,
		options: PreviewOptions
	): Promise<HTMLElement | null> {
		const cardTypeId = previewData.cardType;
		const renderer = Array.from(this.renderers.values()).find((r) =>
			r.supportedTypes.includes(cardTypeId)
		);
