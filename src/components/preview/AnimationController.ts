...
export class AnimationController {
	...
	animateHover(element: HTMLElement | null, isEntering: boolean): Promise<void> {
		...
		const keyframes = isEntering
			? [{ transform: "translateY(0) scale(1)" }, { transform: "translateY(-2px) scale(1.01)" }]
	...
	animateLoading(element: HTMLElement): Animation | null {
		...
		const animation = element.animate([{ opacity: 0.6 }, { opacity: 1 }, { opacity: 0.6 }], {
			duration: 1500,
			easing: "ease-in-out",
			iterations: Infinity,
		});
	...
	private shouldAnimate(): boolean {
		...
		if (this.options.performanceMode === "performance" && this.animationQueue.length > 5)
			return false;
