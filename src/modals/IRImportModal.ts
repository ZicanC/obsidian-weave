...
export class IRImportModal extends Modal {
	...
	private filterItems(items: FileItem[], query: string): FileItem[] {
		...
		return items.filter((_item) => {
			const nameMatch = _item.name.toLowerCase().includes(lowerQuery);
			const childMatch =
				_item.children.length > 0 && this.filterItems(_item.children, query).length > 0;
	...
	private renderItems(items: FileItem[], container: HTMLElement, depth: number) {
		for (const item of items) {
			...
			// 名称
			itemEl.createSpan({ cls: "ir-file-name", text: item.name });

			// 文件数（仅文件夹）
			if (item.isFolder && item.fileCount > 0) {
				itemEl.createSpan({ cls: "ir-file-count", text: String(item.fileCount) });
			}
			...
			if (item.isFolder && item.fileCount > 0) {
				itemEl.addEventListener("click", (event) => {
					...
					if (target instanceof HTMLElement && target.closest(".ir-expand-btn")) {
						return;
					}
