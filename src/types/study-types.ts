...
export function isPersistedStudySession(value: unknown): value is PersistedStudySession {
	...
	if (
		...
		!isFiniteNumber(value.currentCardIndex) ||
		typeof value.currentCardId !== "string" ||
		!isStringArray(value.remainingCardIds) ||
		!isFiniteNumber(value.startTime) ||
		typeof value.isPaused !== "boolean" ||
		!isPersistedSessionType(value.sessionType)
	) {
		return false;
	}
	...
	if (value.mode !== undefined && !isStudyMode(value.mode)) {
		return false;
	}

	if (value.pauseTime !== undefined && !isFiniteNumber(value.pauseTime)) {
		return false;
	}
	...
	if (
		!isFiniteNumber(value.stats.completed) ||
		!isFiniteNumber(value.stats.correct) ||
		!isFiniteNumber(value.stats.incorrect)
	) {
		return false;
	}

	if (value.queueState !== undefined) {
		if (!isRecord(value.queueState)) {
			return false;
		}

		if (
			!isFiniteNumber(value.queueState.currentCardIndex) ||
			!isStringArray(value.queueState.studyQueueCardIds) ||
			!isStringArray(value.queueState.sessionStudiedCardIds)
		) {
			return false;
		}
	}
