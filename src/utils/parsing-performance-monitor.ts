...
export class ParsingPerformanceMonitor {
	...
	private calculateTrends(): PerformanceReport["trends"] {
		...
		return {
			...
			cacheEfficiencyChange:
				((recentCacheRate - previousCacheRate) / (previousCacheRate || 1)) * 100,
			successRateChange:
				((recentSuccessRate - previousSuccessRate) / (previousSuccessRate || 1)) * 100,
		};
	}
			...
			this.performanceObserver = new PerformanceObserver((list) => {
				...
				entries.forEach((_entry) => {
					if (_entry.entryType === "measure") {
						this.recordOperation(`system_${_entry.name}`, _entry.duration, true, 1, false);
					}
				});
			});
