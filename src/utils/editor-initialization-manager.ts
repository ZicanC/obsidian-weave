...
export class EditorInitializationManager {
	...
	async safeInitialize(
		...
		const activeProcesses = Array.from(this.processes.values()).filter(
			(p) => p.state === InitializationState.INITIALIZING
		);

		if (activeProcesses.length >= this.maxConcurrentInitializations) {
			logger.warn(
				`EditorInitManager: 达到最大并发初始化限制 (${this.maxConcurrentInitializations})`
			);
			return {
				success: false,
	...
	private async executeInitialization(
		...
		while (retryCount <= maxRetries) {
			try {
				...
				// 执行初始化
				logger.debug(
					`EditorInitManager: 开始初始化 [${editorId}] (尝试 ${retryCount + 1}/${maxRetries + 1})`
				);
				...
				logger.error(
					`EditorInitManager: 初始化失败 [${editorId}] (尝试 ${retryCount}/${maxRetries + 1}):`,
					error
				);
