...
export const assert = {
	 ...
	 * 断言类型
	 */
	isType(value: any, type: TypeofName, message?: string): void {
		const actualType = typeof value;
		let matchesType = false;

		switch (type) {
			case "bigint":
				matchesType = actualType === "bigint";
				break;
			case "boolean":
				matchesType = actualType === "boolean";
				break;
			case "function":
				matchesType = actualType === "function";
				break;
			case "number":
				matchesType = actualType === "number";
				break;
			case "object":
				matchesType = actualType === "object";
				break;
			case "string":
				matchesType = actualType === "string";
				break;
			case "symbol":
				matchesType = actualType === "symbol";
				break;
			case "undefined":
				matchesType = actualType === "undefined";
				break;
		}

		if (!matchesType) {
			throw new AssertionError(
				message || `Expected ${value} to be of type ${type}, got ${actualType}`
			);
		}
	},
	...
	contains(container: any, item: any, message?: string): void {
		if (Array.isArray(container)) {
			if (!container.includes(item)) {
				throw new AssertionError(message || `Expected array to contain ${item}`);
			}
		} else if (typeof container === "string") {
			if (!container.includes(item)) {
				throw new AssertionError(message || `Expected string to contain ${item}`);
			}
		} else {
...
export class TestRunner {
	...
	private async runTest(
		test: TestCase,
		hooks: { beforeEach?: () => Promise<void> | void; afterEach?: () => Promise<void> | void }
	): Promise<TestResult> {
