    ...
    try {
      ...
      if (cardId) {
        ...
        if (!studyView) {
          ...
          console.debug('💡 请先打开学习界面，或指定卡片ID');
          return null;
        }
      ...
      console.debug('📇 卡片ID:', result.cardId);
      console.debug('🏷️ 卡片类型:', result.cardType);
      console.debug('📋 V2格式:', result.isV2Format ? '✅ 是' : '❌ 否');
      
      if (result.isParent) {
        console.debug('👨‍👧‍👦 角色: 父卡片 (ProgressiveParent)');
        console.debug('  └─ 子卡片数量:', result.totalClozes);
        console.debug('  └─ 子卡片IDs:', result.childCardIds);
      } else if (result.isChild) {
        console.debug('👶 角色: 子卡片 (ProgressiveChild)');
        console.debug('  └─ 父卡片ID:', result.parentCardId);
        console.debug('  └─ 挖空序号 (clozeOrd):', result.clozeOrd, '(0-based)');
        console.debug('  └─ 激活序号 (activeClozeOrdinal):', result.activeClozeOrdinal, '(1-based)');
      } else {
        console.debug('📄 角色: 普通卡片或V1.5格式');
      }
      ...
      console.group('📝 内容分析');
      console.debug('原始内容:', result.content.substring(0, 100) + '...');
      console.debug('检测到的挖空:', result.clozeMatches.length);
      ...
      if (result.suggestions.length > 0) {
        console.group('💡 建议');
        result.suggestions.forEach(suggestion => console.debug('  •', suggestion));
        console.groupEnd();
      }
