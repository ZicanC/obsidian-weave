	...
	try {
		...
		if (!content || !content.trim()) {
			const availableFields = Object.keys(fields).filter((_k) => {
				...
				return (
					!["templateId", "templateName", "notes"].includes(_k) &&
					value &&
					typeof value === "string" &&
					value.trim()
				);
			...
			if (availableFields.length > 0) {
				logger.debug(
					`handleDefaultFieldContent: using first available field: ${availableFields[0]}`
				);
		...
		// 应用挖空处理
		const processedContent = processClozeText(
			content,
			side,
			showAnswer,
			plugin,
			options.activeClozeOrdinal
		);
...
function handleTemplateFieldContent(
	...
	try {
		...
		logger.debug(
			`handleTemplateFieldContent: found ${relevantFields.length} relevant fields for ${side}`
		);

		if (relevantFields.length === 0) {
			logger.debug(
				`handleTemplateFieldContent: no relevant fields found for ${side}, using fallback`
			);
