...
export class QuestionBankHierarchyService {
  ...
  private buildTreeFromBanks(allBanks: Deck[]): DeckTreeNode[] {
    ...
    const roots: DeckTreeNode[] = allBanks.map(bank => ({
      deck: bank,
      children: [] // 平级架构
    }));
   ...
   * 调用此方法将抛出错误
   */
  async syncMove(_memoryDeckId: string, _newParentMemoryDeckId: string | null): Promise<void> {
  ...
  private async getDescendants(questionBankId: string): Promise<Deck[]> {
    ...
    const collect = (parentPath: string) => {
      const children = allBanks.filter(b => 
        b.parentId && b.path.startsWith(`${parentPath}::`)
      );
