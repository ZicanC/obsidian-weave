...
export class AccuracyCalculator {
  ...
  private readonly BASE_ALPHA = 0.2;
  
  /** 模式权重：不同测试模式的重要性系数 */
  private readonly MODE_WEIGHTS: Record<TestMode, number> = {
  ...
  /** 先验掌握度：初始假设（无数据时） */
  private readonly PRIOR_MASTERY = 0.5;
  
  /** 置信度参数：控制样本量到置信度的映射 */
  private readonly CONFIDENCE_SCALE = 20; // n=20时置信度约63%
   ...
   * - 平滑变化：避免单次测试的剧烈波动
   */
  private calculateEWMA(history: TestAttempt[]): number {
   ...
   * 简单平均正确率（保留用于对比）
   */
  private calculateSimpleAverage(history: TestAttempt[]): number {
  ...
  private determineStatus(
    ...
    if (accuracy >= 0.60) return 'learning';
    if (accuracy >= 0.40) return 'struggling';
    return 'needs_review';
  }
   ...
   * - 差异<10%：稳定
   */
  private analyzeTrend(history: TestAttempt[]): TrendInfo {
    if (history.length < 5) {
   ...
   * 获取连续正确次数（从最近往前数）
   */
  private getConsecutiveCorrect(history: TestAttempt[]): number {
    let count = 0;
   ...
   * 计算最近N次的正确率
   */
  private calculateRecentAccuracy(history: TestAttempt[], n: number): number {
   ...
   * 转换为百分制（保留1位小数）
   */
  private toPercentage(ratio: number): number {
   ...
   * 获取默认指标（无数据时）
   */
  private getDefaultMetrics(): MasteryMetrics {
    return {
      currentAccuracy: 0,
