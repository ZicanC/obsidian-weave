...
import type { QuestionBankFilter, QuestionBankStats, QuestionRef, QuestionTestStats } from '../../types/question-bank-types';
...
export class QuestionBankService {
  ...
  private async loadData(): Promise<void> {
    ...
    //  调试日志：显示从存储加载的原始数据
    logger.debug('[QuestionBankService] loadData 从存储加载的题库:', 
      this.banks.map(b => ({
        id: b.id,
        name: b.name,
    ...
    if (!Array.isArray(this.banks)) {
      logger.error('[QuestionBankService] ⚠️  banks 数据损坏，重置为空数组');
      this.banks = [];
   ...
   */
  async createQuestionBank(name: string, description = '', initialMetadata?: Record<string, any>): Promise<Deck> {
   ...
   * 根据ID获取题库（别名方法）
   */
  async getBankById(bankId: string): Promise<Deck | null> {
  ...
  async findBankByMemoryDeckId(memoryDeckId: string): Promise<Deck | null> {
    ...
    const bank = allBanks.find(b => 
      b.metadata && 
      (b.metadata as any).pairedMemoryDeckId === memoryDeckId
    );
  ...
  async deleteQuestionBank(bankId: string): Promise<void> {
    ...
    const pairedMemoryDeckId = (bank.metadata as any)?.pairedMemoryDeckId as string | undefined;
    const bankIdsToDelete = pairedMemoryDeckId
      ? this.banks.filter((b) => (b.metadata as any)?.pairedMemoryDeckId === pairedMemoryDeckId).map((b) => b.id)
      : [bankId];
   ...
   * 获取题目
   */
  getQuestion(questionId: string): Card | undefined {
    return undefined;
  }
   ...
   * 按标签筛选题目
   */
  getQuestionsByTag(tag: string): Card[] {
    return [];
  }
   ...
   * 按难度筛选题目
   */
  getQuestionsByDifficulty(difficulty: 'easy' | 'medium' | 'hard'): Card[] {
    return [];
  }
  ...
  private applyFilter(questions: Card[], filter: QuestionBankFilter): Card[] {
    ...
    if (filter.untestedOnly) {
      result = result.filter(q => 
        !q.stats.testStats || q.stats.testStats.totalAttempts === 0
      );
    }
  ...
  private async updateBankStats(bankId: string): Promise<void> {
    ...
    const testedQuestions = questions.filter(q => 
      q.stats.testStats && q.stats.testStats.totalAttempts > 0
    ).length;
    
    // 统计平均正确率和分数
    const testedQuestionsWithStats = questions.filter(q => 
      q.stats.testStats && q.stats.testStats.totalAttempts > 0
    );
    ...
    // 统计错题数量
    const errorCount = questions.filter(q => 
      q.stats.testStats?.isInErrorBook
    ).length;
