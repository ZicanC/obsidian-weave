...
import type { PersistedTestSession, PersistedTestQuestionRecord, TestHistoryEntry, TestSession, TestQuestionRecord, TestMode, TestAttempt, QuestionTestStats } from '../../types/question-bank-types';
...
export class TestSessionManager {
    ...
    const session: TestSession = {
      ...
      questions: selectedQuestions.map(q => {
        ...
        try {
          ...
          logger.warn(`[⚠️] 题目 ${q.uuid} 格式错误:`, errorMessage);
        }
   ...
   * 构建测试历史记录（用于EWMA算法）
   */
  private buildTestHistory(question: Card, currentRecord: TestQuestionRecord): TestAttempt[] {
    if (!this.currentSession) {
      return [];
    }
    ...
    // 从当前会话的所有测试记录中提取该题目的历史
    for (const record of this.currentSession.questions) {
      if (record.questionId === question.uuid && record.isCorrect !== null) {  //  修复：使用uuid而非id
        attempts.push({
          isCorrect: record.isCorrect,
          mode: this.currentSession.mode,
          timestamp: record.submittedAt || new Date().toISOString(),
          score: record.isCorrect ? 100 : 0,
          timeSpent: record.timeSpent * 1000 // 转换为毫秒
        });
      }
    }

    // 确保包含当前这次测试
    if (currentRecord.isCorrect !== null && 
        !attempts.some(a => a.timestamp === currentRecord.submittedAt)) {
      attempts.push({
        isCorrect: currentRecord.isCorrect,
        mode: this.currentSession.mode,
        timestamp: currentRecord.submittedAt || new Date().toISOString(),
        score: currentRecord.isCorrect ? 100 : 0,
        timeSpent: currentRecord.timeSpent * 1000 // 转换为毫秒
      });
    }
