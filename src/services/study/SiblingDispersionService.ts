...
export class SiblingDispersionService {
	 ...
	 * @returns 调整后的due日期
	 */
	public async findDispersedDue(card: Card, optimalDue: Date, siblings?: Card[]): Promise<Date> {
		// 检查是否启用和需要调整
		...
		try {
			...
			const siblingDues = siblings
				.filter(
					(s): s is ProgressiveClozeChildCard & { fsrs: NonNullable<typeof s.fsrs> } => !!s.fsrs
				)
			...
			if (bestCandidate.offset !== 0) {
				logger.debug(
					`[SiblingDispersion P3] 调整due日期: ${optimalDue.toISOString().split("T")[0]} → ${
						bestCandidate.date.toISOString().split("T")[0]
					} (偏移${bestCandidate.offset}天, fuzz范围±${fuzzDays}天, 最小间隔${
						bestCandidate.minDistance
					}天)`
				);
			}
