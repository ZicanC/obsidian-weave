...
export class DataSyncService {
	...
	async notifyChange(event: Omit<DataChangeEvent, "timestamp">): Promise<void> {
		...
		// 并行通知所有订阅者
		await Promise.all(subs.map((sub) => this.notifySubscription(sub, fullEvent)));
	}
	 ...
	 * @param event 数据变更事件
	 */
	private async executeCallback(subscription: Subscription, event: DataChangeEvent): Promise<void> {
		try {
