...
export class FilterManager {
	...
	private deckResolver?: () => Array<{ id: string; name: string }>;

	constructor(deckResolver?: () => Array<{ id: string; name: string }>) {
		this.storage = this.loadStorage();
		this.deckResolver = deckResolver;
		this.initializeBuiltInFilters();
	}

	setDeckResolver(deckResolver?: () => Array<{ id: string; name: string }>): void {
		this.deckResolver = deckResolver;
	}
	 ...
	 */
	saveFilter(
		filter: Omit<SavedFilter, "id" | "createdAt" | "updatedAt" | "useCount" | "isBuiltIn">
	): SavedFilter {
	...
	applyFilter(cards: Card[], config: FilterConfig, allFieldTemplates: any[] = []): Card[] {
		...
		return cards.filter((card) => this.evaluateFilterConfig(card, config, allFieldTemplates));
	}
