...
import type { AIAction, GeneratedTestQuestion, TestGenerationResponse } from "../../types/ai-types";
...
export class AITestGeneratorService {
	...
	private buildPrompts(card: Card, action: AIAction): { systemPrompt: string; userPrompt: string } {
		...
		if (testConfig) {
			userPrompt = userPrompt
				...
				.replace(/\{\{类型\}\}/g, getQuestionTypeLabel(testConfig.questionType, "long", "测试题"))
				.replace(/\{\{难度\}\}/g, this.getDifficultyLabel(testConfig.difficultyLevel));
		}

		// 最后解析其他标准变量
		userPrompt = AITestGeneratorService.variableResolver.resolve(userPrompt, card, context);
	...
	private parseAIResponse(responseContent: string, action: AIAction): GeneratedTestQuestion[] {
		try {
			...
			// 首先尝试解析JSON格式
			const jsonMatch =
				cleaned.match(/\[[\s\S]*\]/) || cleaned.match(/\{[\s\S]*"questions"[\s\S]*\}/);
			if (jsonMatch) {
				try {
	...
	private parseJSONResponse(parsed: any, action: AIAction): GeneratedTestQuestion[] {
		...
		for (const [index, q] of questionsArray.entries()) {
			if (
				action.testConfig?.questionType === "multiple" ||
				action.testConfig?.questionType === "single"
			) {
				// 选择题 - 严格验证
				...
				const answer =
					typeof q.answer === "boolean" ? (q.answer ? "正确" : "错误") : q.answer || q.back || "";
				const explanation = q.explanation || "";
	...
	private parseTextResponse(textContent: string, action: AIAction): GeneratedTestQuestion[] {
		...
		try {
			...
			if (
				...
				const answerMatch =
					answerPart.match(/(?:正确答案|答案)[：:]\s*([A-D])/i) || answerPart.match(/^([A-D])\)/m);
				...
				// 其他类型的题目，使用简单的问答格式
				//  构建content字段（权威格式）
				const contentMarkdown = `Q: ${questionPart}\n\nA: ${answerPart}`;
		...
		try {
			...
			for (const question of questions) {
				...
				const newCard = createTestCard({
					...
					content: content,
					difficulty:
						(question.difficulty === "mixed" ? "medium" : question.difficulty) || "medium",
	...
	async getOrCreateQuestionBankDeck(sourceDeckId: string): Promise<string> {
		try {
			...
			// 禁用旧方案：不允许 AI 自动创建题库。仅允许使用“从记忆卡片选择题引入组建考试题组”的新流程。
			// 这里仅允许查找已存在的绑定题库。
			const existingBank =
				await this.plugin.questionBankService.findBankByMemoryDeckId(sourceDeckId);
			if (existingBank) {
				return existingBank.id;
			}

			throw new Error(
				"未找到对应考试题组。该功能已禁用自动创建题组，请在卡片管理中使用“组建考试题组”从选择题引入创建。"
			);
		} catch (error) {
