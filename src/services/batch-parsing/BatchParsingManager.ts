...
import { App, Notice, Modal, Command, TFile } from 'obsidian';
...
import { SimplifiedParsingSettings, ParsedCard } from '../../types/newCardParsingTypes';
...
export class BatchParsingManager {
  ...
  constructor(
    ...
    if (parsingSettings.batchParsing?.folderDeckMappings && 
      ...
      this.config.folderDeckMappings = parsingSettings.batchParsing.folderDeckMappings;
      this.logDebug('从 settings 恢复映射配置:', 
        this.config.folderDeckMappings.length, '个映射');
    } else {
  ...
  async updateConfig(updates: Partial<SimpleBatchParsingConfig>): Promise<void> {
    ...
    if (updates.folderDeckMappings && this.plugin?.settings?.simplifiedParsing?.batchParsing) {
      ...
      this.logDebug('同步映射配置到 settings:', 
        updates.folderDeckMappings.length, '个映射');
  ...
  private syncConfigFromSettings(): void {
    ...
    if (settingsMappings && Array.isArray(settingsMappings) && settingsMappings.length > 0) {
      ...
      this.logDebug('已从 settings 同步最新映射配置:', 
        settingsMappings.length, '个映射', 
        oldCount !== settingsMappings.length ? `(变化: ${oldCount} → ${settingsMappings.length})` : '(无变化)');
