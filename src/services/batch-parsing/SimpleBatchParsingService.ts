...
export class SimpleBatchParsingService {
  ...
  constructor(
    ...
    app: IObsidianApp,
    dataStorage?: WeaveDataStorage,  // 🆕 添加dataStorage参数（用于ThreeWayMergeEngine和SingleCardSyncEngine）
    cleanupService?: BlockLinkCleanupService,  //  添加清理服务参数
    plugin?: any  // 🆕 添加plugin参数（用于ThreeWayMergeEngine访问directFileReader）
  ) {
    this.config = config;
  ...
  async executeBatchParsing(
    ...
    try {
      ...
      if (files.length > this.config.maxFilesPerBatch) {
        new Notice(
          `文件数量超过限制，将只处理前 ${this.config.maxFilesPerBatch} 个文件`,
          5000
        );
      }
  ...
  private async processFile(file: TFile): Promise<{
    ...
    // 获取配置的标签
    const excludeTags = this.config.parsingSettings.batchParsing?.excludeTags || [];
    ...
    for (const card of parseResult) {
      try {
        ...
        // 优先级1: 检查排除标签
        const hasExcludeTag = tags.some(tag => excludeTags.includes(tag));
        if (hasExcludeTag) {
          result.skippedCards!++;
        ...
        // 优先级2: 检查强制同步标签
        const hasForceSyncTag = tags.some(tag => forceSyncTags.includes(tag));
        if (hasForceSyncTag) {
          // 强制同步：无论如何都标记为更新
          if (uuid) {
            result.updatedCards!++;
