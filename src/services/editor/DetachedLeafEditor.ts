...
import { App, Component, MarkdownView, TFile, WorkspaceLeaf, Platform, Scope } from 'obsidian';
...
export class DetachedLeafEditor extends Component {
  ...
  private setupEditor() {
    ...
    if (globalLivePreview) {
        ...
        // 强制切换到 Live Preview
        // @ts-ignore
        const state: any = (this.editorView as any).currentMode;
  ...
  destroy() {
      ...
      if (tempFileToDelete instanceof TFile) {
          ...
          if (isPluginTemp) {
              try {
                  this.app.fileManager.trashFile(tempFileToDelete).catch(err => {
