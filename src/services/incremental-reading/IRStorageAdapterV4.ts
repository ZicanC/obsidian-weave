...
import type { IRBlockV4, IRBlockStatus, IRBlockStats, IRBlockMeta } from '../../types/ir-types';
import { migrateToIRBlockV4, mapStateToStatus, DEFAULT_IR_BLOCK_STATS, DEFAULT_IR_BLOCK_META } from '../../types/ir-types';
...
export class IRStorageAdapterV4 {
   ...
   * 获取所有内容块 (V4格式)
   */
  async getAllBlocksV4(): Promise<Record<string, IRBlockV4>> {
    const blocks = await this.storage.getAllBlocks();
    const result: Record<string, IRBlockV4> = {};
   ...
   * 获取单个内容块 (V4格式)
   */
  async getBlockV4(id: string): Promise<IRBlockV4 | null> {
   ...
   */
  async getBlocksByDeckV4(deckId: string, includeIgnored: boolean = false): Promise<IRBlockV4[]> {
    ...
    if (deck) {
      ...
      const matchedChunks = Object.values(allChunks).filter(chunk => {
        if (typeof (chunk as any).filePath !== 'string' || String((chunk as any).filePath).trim() === '') return false;
        if (chunk.deckTag === deckTag) return true;
        if (chunk.deckIds?.includes(deckId)) return true;
        return false;
      });
  ...
  async getBlocksByDeckV4Fast(deckId: string, includeIgnored: boolean = false): Promise<IRBlockV4[]> {
    const deck = await this.storage.getDeckById(deckId);
    if (deck) {
      const allChunks = await this.storage.getAllChunkData();
      const deckTag = `#IR_deck_${deck.name}`;
   ...
   */
  private chunkToV4(chunk: import('../../types/ir-types').IRChunkFileData, _sourceOriginalPath?: string): IRBlockV4 {
    ...
    const sourcePath = typeof _sourceOriginalPath === 'string' && _sourceOriginalPath.trim().length > 0
      ? _sourceOriginalPath
      : (typeof (chunk as any).filePath === 'string' ? chunk.filePath : '');

    const priorityUi = typeof (chunk as any).priorityUi === 'number' ? (chunk as any).priorityUi : chunk.priorityEff;
    return {
      id: chunk.chunkId,
      sourcePath,
  ...
  private async chunkToV4WithTitle(
    ...
    try {
      if (typeof (chunk as any).filePath !== 'string' || String((chunk as any).filePath).trim() === '') {
  ...
  private extractTagsFromYamlContent(yamlContent: string): string[] {
   ...
   * 获取文件的所有内容块 (V4格式)
   */
  async getBlocksByFileV4(filePath: string): Promise<IRBlockV4[]> {
   ...
   * 保存内容块 (V4格式)
   */
  async saveBlockV4(block: IRBlockV4): Promise<void> {
   ...
   * 批量保存内容块 (V4格式)
   */
  async saveBlocksV4(blocks: IRBlockV4[]): Promise<void> {
   ...
   * 获取到期的 V4 内容块
   */
  async getDueBlocksV4(deckId: string): Promise<IRBlockV4[]> {
    ...
    const dueBlocks = blocks.filter(block => {
      ...
      if (block.nextRepDate > endMs) return false;
      return true;
    });
   ...
   */
  private toV4(block: IRBlock): IRBlockV4 {
   ...
   */
  async migrateAllToV4(): Promise<{ migrated: number; errors: number }> {
    const blocks = await this.storage.getAllBlocks();
    let migrated = 0;
    let errors = 0;
    
    const v3Blocks: IRBlock[] = [];
