...
import type { IRBlockV4, IRBlockStatus, IRPriorityLogEntry } from '../../types/ir-types';
import { 
 ...
 * 有效的状态迁移映射
 */
const VALID_TRANSITIONS: Record<IRBlockStatus, IRBlockStatus[]> = {
  'new': ['queued'],
...
export class IRStateMachineV4 {
   ...
   * @returns 更新后的内容块
   */
  transitionToQueued(block: IRBlockV4, initialIntervalDays: number = 1): IRBlockV4 {
    if (block.status !== 'new') {
  ...
  updatePriority(
    ...
    reason: string,
    useTimeAwareEWMA: boolean = false
  ): IRBlockV4 {
    // 强制理由检查
  ...
  updateStats(
    ...
    extracts: number = 0,
    cardsCreated: number = 0,
    notesWritten: number = 0
  ): IRBlockV4 {
    const now = Date.now();
