...
export class IRStorageService {
  ...
  private async migrateReadablePathIfNeeded(p: unknown, fromRoot: string, toRoot: string): Promise<string | null> {
   ...
   */
  async getBlocksByDeck(deckId: string, includeIgnored: boolean = false, caller: string = 'unknown'): Promise<IRBlock[]> {
    const blocks = await this.getAllBlocks();
    ...
    const filterIgnored = (block: IRBlock): boolean => {
      ...
      const hasIgnoreInTags = block.tags?.some(tag => 
  ...
  async getAllDecks(): Promise<Record<string, IRDeck>> {
    ...
    try {
      ...
      if (data.version && data.decks) {
        if (!this.migratedDeckReadablePaths) {
          ...
          if (roots && data.decks && typeof data.decks === 'object') {
            let changed = false;
            for (const deck of Object.values(data.decks as Record<string, any>)) {
      ...
      if (!this.migratedDeckReadablePaths && data && typeof data === 'object') {
        ...
        if (roots) {
          let changed = false;
          for (const deck of Object.values(data as Record<string, any>)) {
            if (!deck || typeof deck !== 'object') continue;
            const srcFiles = (deck as any).sourceFiles;
            if (Array.isArray(srcFiles)) {
              for (let i = 0; i < srcFiles.length; i++) {
  ...
  async migrateChunkDeckNameInYAML(oldName: string, newName: string): Promise<{ scanned: number; updated: number }> {
    await this.initialize();
    ...
    for (const chunk of Object.values(chunks)) {
      ...
      try {
        ...
        const rawDeckNames = (yaml as any).deck_names;
        ...
        const rawDeckTag = typeof (yaml as any).deck_tag === 'string' ? String((yaml as any).deck_tag) : '';
  ...
  async deleteDeck(idOrPath: string): Promise<void> {
    ...
    // v2.0: 先尝试直接用 id 删除
    if (decks[idOrPath]) {
      delete decks[idOrPath];
    } else {
      // 兼容: 尝试通过 path 字段查找并删除
      const deckKey = Object.keys(decks).find(key => decks[key].path === idOrPath);
      if (deckKey) {
  ...
  async getHistory(): Promise<{ sessions: IRSession[] }> {
    ...
    try {
      ...
      if (data.version && data.sessions) {
        return { sessions: data.sessions as IRSession[] };
      }
      
      // v1.0 兼容
      return data as { sessions: IRSession[] };
    } catch (error) {
   ...
   */
  async getDeckStats(deckPath: string, dailyNewLimit: number = 20, dailyReviewLimit: number = 50, learnAheadDays: number = 3): Promise<{
    newCount: number;
    ...
    const chunkValues = allChunkValues.filter(chunk => {
      ...
      if (hasIgnoreTagInFile(chunk.filePath)) return false;
      return true;
    });
   ...
   */
  private async readFile(path: string, defaultContent: string = '{}'): Promise<string> {
    try {
      const adapter = this.app.vault.adapter;
   ...
   */
  async checkFileNeedsSync(filePath: string, currentMtime: number, currentSize: number): Promise<boolean> {
   ...
   * 获取单个源材料元数据
   */
  async getSource(sourceId: string): Promise<import('../../types/ir-types').IRSourceFileMeta | null> {
  ...
  async saveSourceBatch(sourceList: import('../../types/ir-types').IRSourceFileMeta[]): Promise<void> {
    await this.initialize();
    const sources = await this.getAllSources();
  ...
  async getAllChunkData(): Promise<Record<string, import('../../types/ir-types').IRChunkFileData>> {
    ...
    for (const [key, chunk] of Object.entries(parsed.chunks)) {
      ...
      if ((chunk as any).priorityUi === undefined && typeof (chunk as any).priorityEff === 'number') {
      ...
      try {
        ...
        const sourceTagGroup = sourceId && sources ? (sources as any)[sourceId]?.tagGroup : undefined;
   ...
   * 获取单个块文件调度数据
   */
  async getChunkData(chunkId: string): Promise<import('../../types/ir-types').IRChunkFileData | null> {
    const chunks = await this.getAllChunkData();
  ...
  async saveChunkData(chunk: import('../../types/ir-types').IRChunkFileData): Promise<void> {
    ...
    const store: import('../../types/ir-types').IRChunksStore = {
      version: IR_STORAGE_VERSION,
      chunks
    };
   ...
   * 批量保存块文件调度数据
   */
  async saveChunkDataBatch(chunkList: import('../../types/ir-types').IRChunkFileData[]): Promise<void> {
   ...
   * 获取源材料的所有块调度数据
   */
  async getChunkDataBySource(sourceId: string): Promise<import('../../types/ir-types').IRChunkFileData[]> {
    const chunks = await this.getAllChunkData();
  ...
  async getActiveChunkData(): Promise<import('../../types/ir-types').IRChunkFileData[]> {
    const chunks = await this.getAllChunkData();
    return Object.values(chunks).filter(c => 
      c.scheduleStatus !== 'done' && c.scheduleStatus !== 'suspended' && c.scheduleStatus !== 'removed'
    );
  }
  ...
  async getTodayDueChunkData(): Promise<import('../../types/ir-types').IRChunkFileData[]> {
    ...
    return Object.values(chunks).filter(chunk => {
      // 新块立即可用
      if (chunk.scheduleStatus === 'new') return true;
      // 已完成或暂停的跳过
   ...
   */
  async getChunksByDeckTag(deckTag: string): Promise<import('../../types/ir-types').IRChunkFileData[]> {
    const chunks = await this.getAllChunkData();
   ...
   */
  private async updateChunkFileYAML(filePath: string, updates: Record<string, unknown>): Promise<void> {
    const adapter = this.app.vault.adapter;
   ...
   * 根据牌组标签获取统计数据
   */
  async getDeckStatsByTag(deckTag: string, dailyNewLimit: number = 20, dailyReviewLimit: number = 50): Promise<{
    newCount: number;
    learningCount: number;
    reviewCount: number;
    dueToday: number;
   ...
   */
  async getAllChunkDataWithSync(): Promise<Record<string, import('../../types/ir-types').IRChunkFileData>> {
    // 先同步牌组数据（包含验证）
    await this.syncDeckDataFromYAML();
    // 返回最新数据
  ...
  async slimIRMarkdownFrontmatter(scanRoot?: string): Promise<{ updated: number; scanned: number }> {
    await this.initialize();
    const adapter = this.app.vault.adapter;
  ...
  async cleanupInvalidBlocks(): Promise<{ removed: number; invalidPaths: string[] }> {
    ...
    for (const [blockId, block] of Object.entries(blocks)) {
      if (typeof (block as any).filePath !== 'string' || String((block as any).filePath).trim() === '') {
   ...
   * 先清理无效数据，再返回有效数据
   */
  async getAllChunkDataWithIntegrityCheck(scanRoot?: string): Promise<Record<string, import('../../types/ir-types').IRChunkFileData>> {
    // 先执行完整性检查
