...
export interface SplitResult {
  headingText: string;
  headingPath: string[];      // v2.0: 标题层级路径
  headingLevel: number;
  content: string;
  startLine: number;
  endLine: number;
...
export class IRContentSplitter {
  ...
  async splitFile(
    ...
    for (let i = 0; i < splits.length; i++) {
      ...
      if (blockId && existingIdToBlock.has(blockId)) {
        ...
        (block as any).endLine = split.endLine;
        // 兼容字段
   ...
   */
  private splitByHeading(lines: string[], splitLevel: number): SplitResult[] {
   ...
   */
  private splitByParagraph(lines: string[]): SplitResult[] {
    const results: SplitResult[] = [];
   ...
   * 两个 ---IR--- 之间的内容为一个内容块
   */
  private splitByManualMarkers(lines: string[]): SplitResult[] {
    const results: SplitResult[] = [];
   ...
   * 连续两个或以上空行作为分隔符
   */
  private splitByBlankLines(lines: string[]): SplitResult[] {
    const results: SplitResult[] = [];
   ...
   */
  private splitByCustomMarker(lines: string[], marker: string): SplitResult[] {
    const results: SplitResult[] = [];
   ...
   * 从内容行中提取标题
   */
  private extractBlockTitleFromLines(lines: string[]): string {
   ...
   */
  private extractExistingBlockIds(content: string): string[] {
    const ids: string[] = [];
   ...
   */
  private findBlockIdAtPosition(lines: string[], startLine: number): string | null {
    // 检查起始行前后5行
  ...
  async injectInitialMarkers(
    ...
    for (let i = 0; i < sortedBlocks.length; i++) {
      ...
      const endLine = (block as any).endLine ?? startLine;
   ...
   */
  splitByUuidMarkers(lines: string[], startLineOffset: number = 0): SplitResult[] {
    const results: SplitResult[] = [];
    let currentContent: string[] = [];
   ...
   */
  private extractBlockTitle(lines: string[]): string {
    for (const line of lines) {
      const trimmed = line.trim();
      if (trimmed.length === 0) continue;
   ...
   */
  async injectBlockMarkers(file: TFile, blocks: IRBlock[]): Promise<void> {
    let content = await this.app.vault.read(file);
    const lines = content.split('\n');
    
    // 按 endLine 排序（从后往前注入，避免行号偏移）
    const sortedBlocks = [...blocks].sort((a, b) => 
      ((b as any).endLine ?? b.startLine ?? b.blockIndex ?? 0) - 
      ((a as any).endLine ?? a.startLine ?? a.blockIndex ?? 0)
    );
    ...
    for (const block of sortedBlocks) {
      ...
      const endLine = (block as any).endLine ?? (block.startLine ?? 0) + 1;
   ...
   */
  async removeBlockMarkers(file: TFile): Promise<void> {
    let content = await this.app.vault.read(file);
   ...
   * 获取内容块的实际内容
   */
  async getBlockContent(block: IRBlock): Promise<string> {
