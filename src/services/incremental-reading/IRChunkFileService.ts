...
import {
	...
	type IRIndexFileYAML,
	type IRSourceFileMeta,
	createDefaultChunkFileData,
	createDefaultChunkFileYAML,
	createDefaultIndexFileYAML,
	generateChunkId,
	generateSourceId,
} from "../../types/ir-types";
import {
	deriveFileTitleFromContent,
	parseManualSplitMarkers,
	splitByRules,
} from "../../utils/content-split-utils";
...
export class IRChunkFileService {
	...
	private async getUniqueFolderPath(folderPath: string): Promise<string> {
	...
	async ensureDirectories(): Promise<void> {
		...
		for (const dir of dirs) {
			...
			if (!folder) {
				try {
					await DirectoryUtils.ensureDirRecursive(this.app.vault.adapter, normalizedPath);
	 ...
	 * @returns 导入结果
	 */
	async importFileAsChunks(
		sourceFile: TFile,
		options: ChunkFileImportOptions & { bookFolderName?: string }
		...
		let articleFolderPathBase: string;
		if (options.bookFolderName) {
			const bookFolder = normalizePath(
				`${this.chunkRoot}/${sanitizeFileName(options.bookFolderName)}`
			);
		...
		// 拆分内容
		const contentBlocks = splitByRules(bodyContent, options.splitConfig, {
			defaultTitle: sourceFile.basename,
		});
		...
		const deckIndexPathForBacklink =
			options.deckNames && options.deckNames.length > 0
		...
		// 4. 生成块文件
		const chunkDataList: IRChunkFileData[] = [];
		...
		for (let i = 0; i < contentBlocks.length; i++) {
			...
			// 生成块文件（传入索引文件路径用于回链）
			const chunkFilePath = await this.createChunkFile(
				chunkId,
				sourceId,
				block,
				i,
				options.initialPriority,
				articleFolderPath,
				deckIndexPathForBacklink || undefined,
				options.deckTag,
				options.deckNames
			);
		...
		const sourceMeta: IRSourceFileMeta = {
			sourceId,
	...
	async importFileAsChunksFromBlocks(
		sourceFile: TFile,
	 ...
	 * 复制源文件到 IR/raw/
	 */
	private async copyToRaw(sourceFile: TFile, sourceId: string, title: string): Promise<string> {
		const sanitizedTitle = sanitizeFileName(title);
	...
	private async createChunkFile(
		...
		// 构建文件内容
		const yamlContent = yamlStringify(yamlData as unknown as Record<string, unknown>);
		const fileContent = `---\n${yamlContent}\n---\n${backLink}\n${block.content}`;
	 ...
	 */
	private async createIndexFileInFolder(
		sourceId: string,
		title: string,
		chunkIds: string[],
		chunkFilePaths: string[],
		...
		// 构建块清单（双链格式）
		const chunkListLines: string[] = ["## 内容块清单\n"];
	...
	async updateChunkFileYAML(filePath: string, updates: Partial<IRChunkFileYAML>): Promise<boolean> {
		try {
			...
			const newYamlContent = yamlStringify(updatedYAML as unknown as Record<string, unknown>);
	 ...
	 * 将块状态标记为完成
	 */
	async markChunkAsDone(filePath: string): Promise<boolean> {
	 ...
	 * 检查块文件是否处于完成状态
	 */
	async isChunkDone(filePath: string): Promise<boolean> {
	 ...
	 * 获取所有活跃状态的块文件路径
	 */
	async getActiveChunkFiles(): Promise<string[]> {
		const chunksFolder = this.app.vault.getAbstractFileByPath(this.chunkRoot);
		if (!(chunksFolder instanceof TFolder)) return [];
	...
	async ensureDeckIndexCard(deckName: string): Promise<string> {
		await this.ensureDirectories();
		...
		const content = [
			...
			`FROM "${this.chunkRoot}"`,
			'WHERE weave_type = "ir-chunk" AND (contains(tags, this.deck_tag) OR contains(tags, this.topic_tag) OR contains(topic_names, this.topic_name) OR contains(deck_names, this.deck_name))',
			"SORT file.name ASC",
			"```",
			"",
		].join("\n");
	...
	async renameDeckIndexCard(oldName: string, newName: string): Promise<string> {
		await this.ensureDirectories();
	...
	private async ensureDeckInMasterIndex(deckName: string, deckIndexPath: string): Promise<void> {
		const masterIndexPath = this.masterIndexPath;
		...
		if (existingContent?.includes(deckLink)) {
			return;
		}
		...
		if (existingContent.includes("## 牌组")) {
			...
			existingContent =
				existingContent.substring(0, insertAt) +
				`\n\n${deckLink}` +
				existingContent.substring(insertAt);
		} else {
			existingContent += `\n\n## 牌组\n\n${deckLink}\n`;
		}
	...
	async updateMasterIndex(
		...
		if (existingContent.includes(categoryHeader)) {
			...
			const nextCategoryMatch = existingContent
				.substring(categoryIndex + categoryHeader.length)
				.match(/\n## \[\[/);
			...
			existingContent = `${existingContent.substring(
				0,
				insertPosition
			)}\n${bookEntry}${existingContent.substring(insertPosition)}`;
	 ...
	 * 创建分类文件（可选）
	 */
	async ensureCategoryFile(category: string): Promise<string> {
