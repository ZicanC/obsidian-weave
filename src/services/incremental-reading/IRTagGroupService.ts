...
export class IRTagGroupService {
  ...
  private async loadGroups(): Promise<void> {
    ...
    try {
      ...
      const groups = (parsed && typeof parsed === 'object' && (parsed as any).groups && typeof (parsed as any).groups === 'object')
  ...
  private async loadProfiles(): Promise<void> {
    ...
    try {
      if (!(await adapter.exists(filePath))) {
        ...
        const profiles = (parsed && typeof parsed === 'object' && (parsed as any).profiles && typeof (parsed as any).profiles === 'object')
  ...
  private async loadDocumentMap(): Promise<void> {
    ...
    try {
      ...
      const map = (parsed && typeof parsed === 'object' && (parsed as any).map && typeof (parsed as any).map === 'object')
        ? (parsed as any).map
        : {};
   ...
   * @returns 匹配的标签组 ID
   */
  async matchGroupForDocument(filePath: string, forceRefresh: boolean = false): Promise<string> {
    await this.initialize();
   ...
   * 获取组统计信息
   */
  async getGroupStats(): Promise<Array<{
    group: IRTagGroup;
