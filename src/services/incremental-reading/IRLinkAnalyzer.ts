 ...
 * 默认配置
 */
export const DEFAULT_LINK_ANALYZER_CONFIG: LinkAnalyzerConfig = {
  maxLinkFactor: 0.5,
...
export class IRLinkAnalyzer {
  private app: App;
  private config: LinkAnalyzerConfig;
  
  /** 链接指标缓存 (blockId -> metrics) */
  private metricsCache: Map<string, LinkMetrics> = new Map();
  
  /** 反向链接缓存 (filePath -> metrics) */
  private backlinkCache: Map<string, BacklinkMetrics> = new Map();
  ...
  async analyzeBlocks(
    blocks: IRBlock[],
    getContent: (block: IRBlock) => Promise<string>
   ...
   * 更新所有文件的反向链接缓存
   */
  async updateBacklinkCache(): Promise<void> {
   ...
   */
  private calculateLinkContribution(linkCount: number): number {
   ...
   * 计算重要性得分（简化PageRank）
   */
  private calculateImportanceScore(backlinkCount: number): number {
    if (backlinkCount <= 0) return 0;
 ...
 * 创建链接分析器实例
 */
export function createLinkAnalyzer(
  app: App,
  config?: Partial<LinkAnalyzerConfig>
): IRLinkAnalyzer {
