...
export class ReadingMaterialStorage {
   ...
   * 通过UUID获取阅读材料
   */
  async getMaterialById(uuid: string): Promise<ReadingMaterial | null> {
   ...
   * 通过文件路径获取阅读材料
   */
  async getMaterialByPath(filePath: string): Promise<ReadingMaterial | null> {
   ...
   * 按分类获取材料
   */
  async getMaterialsByCategory(category: string): Promise<ReadingMaterial[]> {
    return Array.from(this.materialsCache.values())
