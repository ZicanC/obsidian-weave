...
export class IRChunkScheduleAdapter {
  ...
  async updateChunkSchedule(
    chunkId: string,
    updates: Partial<Pick<IRChunkFileData, 'priorityUi' | 'priorityEff' | 'intervalDays' | 'nextRepDate' | 'scheduleStatus' | 'doneReason' | 'doneAt'>>
  ): Promise<void> {
  ...
  async batchUpdateChunkSchedules(
    ...
    for (const { chunkId, data } of updates) {
      ...
      if (data.priorityUi !== undefined) chunk.priorityUi = data.priorityUi;
      if (data.priorityEff !== undefined) chunk.priorityEff = data.priorityEff;
      if (data.intervalDays !== undefined) chunk.intervalDays = data.intervalDays;
      if (data.nextRepDate !== undefined) chunk.nextRepDate = data.nextRepDate;
      if (data.scheduleStatus !== undefined) chunk.scheduleStatus = data.scheduleStatus;
      if (data.doneReason !== undefined) chunk.doneReason = data.doneReason;
      if (data.doneAt !== undefined) chunk.doneAt = data.doneAt;
      chunk.updatedAt = Date.now();
