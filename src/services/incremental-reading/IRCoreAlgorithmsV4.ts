...
export const AGING_STRENGTH_CONFIG: Record<'low' | 'medium' | 'high', { slope: number; cap: number }> = {
 ...
 * @returns 新的有效优先级 (0-10)
 */
export function calculatePriorityEWMATimeAware(
  priorityUi: number,
  priorityEffOld: number,
 ...
 */
export function calculatePsi(p: number): number {
 ...
 */
export function calculateNextRepDate(intervalDays: number): number {
  const now = Date.now();
 ...
 * @returns 预估成本（分钟）
 */
export function estimateBlockCost(block: IRBlockV4, defaultMinutes: number = 2): number {
  // 如果有历史数据，使用平均值
 ...
 * @returns 有效时长（秒）
 */
export function calculateEffectiveReadingTime(
  rawSeconds: number,
  wordCount: number,
  speedAvg: number = SPEED_AVG,
  maxRatio: number = 1.5
 ...
 * @returns 是否低质量
 */
export function isLowQualityTime(
  rawSeconds: number,
  wordCount: number,
  outputCount: number,
...
export function checkBankruptcy(
  blocks: IRBlockV4[],
  highPriorityThreshold: number = 8,
  overdueThreshold: number = 7,
  countThreshold: number = 20
