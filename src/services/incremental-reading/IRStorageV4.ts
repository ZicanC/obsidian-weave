...
import { createDefaultIRBlockV4, DEFAULT_IR_BLOCK_STATS, DEFAULT_IR_BLOCK_META } from '../../types/ir-types';
...
interface BlocksStoreV4 {
  version: '4.0';
  blocks: Record<string, IRBlockV4>;
}

interface DecksStoreV4 {
  version: '4.0';
  decks: Record<string, IRDeck>;
}

interface HistoryStoreV4 {
  version: '4.0';
  sessions: IRSession[];
}
...
export class IRStorageV4 {
  ...
  // 内存缓存
  private blocksCache: Record<string, IRBlockV4> = {};
  private decksCache: Record<string, IRDeck> = {};
  private sessionsCache: IRSession[] = [];
  
  // 存储路径
  private readonly basePath: string;
  private readonly blocksFile: string;
  private readonly decksFile: string;
   ...
   * 获取所有内容块
   */
  async getAllBlocks(): Promise<Record<string, IRBlockV4>> {
   ...
   * 获取单个内容块
   */
  async getBlock(id: string): Promise<IRBlockV4 | null> {
    await this.initialize();
   ...
   * 保存内容块
   */
  async saveBlock(block: IRBlockV4): Promise<void> {
    await this.initialize();
   ...
   * 批量保存内容块
   */
  async saveBlocks(blocks: IRBlockV4[]): Promise<void> {
    await this.initialize();
    const now = Date.now();
   ...
   * 获取牌组的所有内容块
   */
  async getBlocksByDeck(deckId: string): Promise<IRBlockV4[]> {
    await this.initialize();
    ...
    if (deck.blockIds && deck.blockIds.length > 0) {
      return deck.blockIds
        ...
        .filter((b): b is IRBlockV4 => b !== undefined)
   ...
   * 获取文件的所有内容块
   */
  async getBlocksByFile(filePath: string): Promise<IRBlockV4[]> {
    await this.initialize();
    return Object.values(this.blocksCache)
   ...
   * 获取到期的内容块
   */
  async getDueBlocks(deckId: string): Promise<IRBlockV4[]> {
    const blocks = await this.getBlocksByDeck(deckId);
    const now = Date.now();
    
    return blocks.filter(block => {
      // 新块总是到期
      if (block.status === 'new') return true;
   ...
   * 获取单个牌组
   */
  async getDeck(idOrPath: string): Promise<IRDeck | null> {
    await this.initialize();
   ...
   */
  async getDeckStats(deckId: string, learnAheadDays: number = 3): Promise<IRDeckStats> {
    const blocks = await this.getBlocksByDeck(deckId);
    const now = Date.now();
  ...
  private async loadBlocks(): Promise<void> {
    try {
      ...
      if (await adapter.exists(this.blocksFile)) {
        ...
        const data: BlocksStoreV4 = JSON.parse(content);
  ...
  private async loadDecks(): Promise<void> {
    try {
      ...
      if (await adapter.exists(this.decksFile)) {
        ...
        const data: DecksStoreV4 = JSON.parse(content);
  ...
  private async loadHistory(): Promise<void> {
    try {
      ...
      if (await adapter.exists(this.historyFile)) {
        ...
        const data: HistoryStoreV4 = JSON.parse(content);
  ...
  private async persistDecks(): Promise<void> {
    try {
      const adapter = this.app.vault.adapter;
      const data: DecksStoreV4 = {
        version: '4.0',
 ...
 * 工厂函数
 */
export function createIRStorageV4(app: App): IRStorageV4 {
