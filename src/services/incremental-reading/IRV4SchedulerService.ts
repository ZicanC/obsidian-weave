...
export class IRV4SchedulerService {
  ...
  async getStudyQueueV4(
    ...
    const allBlocks: IRBlockV4[] = [...chunkBlocks, ...pdfTaskBlocks];

    // maxAppearancesPerDay 过滤：跳过今日已达上限的块
    const maxPerDay = advSettings.maxAppearancesPerDay ?? 2;
    const todayStr = new Date().toISOString().slice(0, 10);
    const blocks = allBlocks.filter(b => {
      if (b.status === 'done' || b.status === 'suspended' || b.status === 'removed') return false;
    ...
    if (postponeStrategy !== 'off' && candidates.length > 0) {
      ...
      if (isOverloaded) {
        ...
        if (cfg) {
          const postponeUpdates: Array<{ chunkId: string; data: { scheduleStatus: IRBlockStatus; nextRepDate: number } }> = [];
    ...
    let generatedQueue = this.queueGenerator.generateQueue(
      candidates,
      groupMapping,
      undefined,
      timeBudgetMinutes,
  ...
  async completeBlockV4(
    ...
    let newPriorityEff = calculatePriorityEWMA(
      newPriorityUi, updatedBlock.priorityEff,
      halfLifeDays, lastInteractionMs > 0 ? lastInteractionMs : undefined
    );
            ...
            if (sourceFile) {
              ...
              if (drift) {
                const storageAdapter = {
                  getChunkData: (id: string) => this.storageService.getChunkData(id) as Promise<any>,
   ...
   * @param deckPath 牌组路径
   */
  async skipBlockV4(blockV4: IRBlockV4, deckPath: string = ''): Promise<void> {
    await this.initialize();
  ...
  async archiveBlockV4(blockV4: IRBlockV4): Promise<IRBlockV4> {
    ...
    await this.recordSession(
      updatedBlock,
      { rating: 0, readingTimeSeconds: 0, priorityUi: updatedBlock.priorityUi, createdCardCount: 0, createdExtractCount: 0, createdNoteCount: 0 },
      '',
      'completed'
    );
   ...
   */
  async deleteBlockV4(blockV4: IRBlockV4, deleteChunkFile: boolean = true): Promise<void> {
    await this.initialize();
   ...
   */
  async skipBlockFromV3(blockV3: IRBlock, deckPath: string = ''): Promise<void> {
    const blockV4 = migrateToIRBlockV4(blockV3);
