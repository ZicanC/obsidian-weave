 ...
 * 交错策略
 */
export enum InterleaveStrategy {
  /** 聚类：相关内容一起学习 */
  CLUSTER = 'cluster',
  /** 交错：相关内容分开学习 */
...
export class IRInterleaveScheduler {
  private config: InterleaveConfig;
   ...
   * @returns 重排后的队列
   */
  applyInterleaving(blocks: IRBlock[], isNewSession: boolean = true): IRBlock[] {
    if (blocks.length <= 1) {
      return blocks;
    }
    ...
    logger.debug(
      `[IRInterleaveScheduler] 应用${strategy}策略, ` +
   ...
   */
  private applyClustering(blocks: IRBlock[]): IRBlock[] {
    // 按标签分组
   ...
   */
  private applyStrictInterleaving(blocks: IRBlock[]): IRBlock[] {
    const groups = this.groupByTopic(blocks);
    ...
    while (result.length < blocks.length && attempts < maxAttempts) {
      ...
      // 找一个与上一个不同的组
      let selectedGroup: BlockGroup | null = null;
   ...
   * 按文件交错（当只有一个主题时的备选方案）
   */
  private interleaveByFile(blocks: IRBlock[]): IRBlock[] {
    const fileGroups = new Map<string, IRBlock[]>();
    ...
    // 转换为数组并交错
    const groups: BlockGroup[] = Array.from(fileGroups.entries()).map(
   ...
   * - 理解度高的内容 → 交错（挑战区分）
   */
  private applyAdaptiveInterleaving(blocks: IRBlock[]): IRBlock[] {
    // 分离低理解度和高理解度块
    const lowComprehension: IRBlock[] = [];
    const highComprehension: IRBlock[] = [];
   ...
   */
  private applyFuzz(blocks: IRBlock[]): IRBlock[] {
    ...
    for (let i = 0; i < result.length - 1; i++) {
      ...
      const swapIdx = Math.min(
        i + 1 + Math.floor(Math.random() * swapRange),
        result.length - 1
      );
   ...
   * 按主题分组（使用标签作为主题标识）
   */
  private groupByTopic(blocks: IRBlock[]): BlockGroup[] {
 ...
 * 创建交错调度器实例
 */
export function createInterleaveScheduler(
  config?: Partial<InterleaveConfig>
): IRInterleaveScheduler {
