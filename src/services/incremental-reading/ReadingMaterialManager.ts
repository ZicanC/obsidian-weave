...
export class ReadingMaterialManager {
  ...
  async createMaterial(
    ...
    // 默认复制文件到导入文件夹
    const shouldCopy = options.copyToImportFolder !== false;
  ...
  async updatePriority(materialId: string, priority: number): Promise<boolean> {
    ...
    if (file) {
      try {
        await this.yamlManager.updatePriority(file as TFile, material.priority);
      } catch (error) {
   ...
   * 获取今日到期的材料
   */
  async getTodayDueMaterials(): Promise<ReadingMaterial[]> {
    return await this.storage.getTodayDueMaterials();
  }

  /**
   * 获取指定分类的材料
   */
  async getMaterialsByCategory(category: ReadingCategory): Promise<ReadingMaterial[]> {
    return await this.storage.getMaterialsByCategory(category);
  }

  /**
   * 获取最近访问的材料
   */
  async getRecentMaterials(limit: number = 5): Promise<ReadingMaterial[]> {
    return await this.storage.getRecentMaterials(limit);
  }

  /**
   * 获取所有材料
   */
  async getAllMaterials(): Promise<ReadingMaterial[]> {
    return await this.storage.getAllMaterials();
  }
  ...
  async removeMaterial(materialId: string): Promise<boolean> {
    ...
    if (success) {
      // 尝试清理YAML frontmatter中的阅读材料字段
      const file = this.app.vault.getAbstractFileByPath(material.filePath);
      if (file) {
        try {
          await this.yamlManager.removeReadingFields(file as TFile);
        } catch (error) {
   ...
   */
  async getExtractCards(deckId?: string): Promise<import('../../types/extract-types').ExtractCard[]> {
    // 从存储中获取所有材料
    const materials = await this.storage.getAllMaterials();
   ...
   * 更新摘录卡片类型
   */
  async updateExtractCardType(cardId: string, newType: import('../../types/extract-types').ExtractType): Promise<boolean> {
   ...
   * 将摘录类型映射回阅读分类
   */
  private mapExtractTypeToCategory(type: import('../../types/extract-types').ExtractType): ReadingCategory {
    switch (type) {
      case 'note':
   ...
   * 将阅读分类映射到摘录类型
   */
  private mapCategoryToExtractType(category: ReadingCategory): import('../../types/extract-types').ExtractType {
    switch (category) {
      case Category.Later:
        return 'note';
   ...
   */
  async addExtractCard(extractCard: import('../../types/extract-types').ExtractCard): Promise<boolean> {
    try {
      // 将摘录卡片转换为阅读材料格式
      const now = new Date().toISOString();
