...
export class ParagraphParser {
   ...
   * 按空行分割文档（原有逻辑）
   */
  private parseByParagraph(content: string): Paragraph[] {
        ...
        if (headingMatch) {
          ...
          if (level === splitLevel) {
            ...
            if (currentLines.length > 0) {
              const paragraph = this.createHeadingParagraph(
                paragraphs.length,
                startLine,
                i - 1,
                startOffset,
  ...
  private parseByManualMarkers(content: string): Paragraph[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine === config.endMarker && inManualBlock) {
        // 结束手动标记块
        if (currentLines.length > 0) {
          const paragraph = this.createManualBlockParagraph(
            paragraphs.length,
            blockStartLine,
            i - 1,
            blockStartOffset,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!inCodeBlock) {
        if (trimmedLine === markerConfig.startMarker) {
          // 保存之前的内容
          if (currentLines.length > 0) {
            const paragraph = this.createCombinedParagraph(
              paragraphs.length,
              startLine,
              i - 1,
              startOffset,
