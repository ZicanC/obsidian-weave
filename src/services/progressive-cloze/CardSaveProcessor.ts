 ...
 * - 功能未发布，无需数据迁移
 *
 * Compatibility note: 请使用 ProgressiveClozeGateway
 ...
 * 挖空分析结果
 */
interface ClozeAnalysisResult {
	isProgressiveCloze: boolean;
	clozeCount: number;
	clozes: Array<{
		ord: number;
		text: string;
...
export class CardSaveProcessor {
	...
	async beforeSave(
		originalCard: Card,
		updatedCard: Card,
		strategy: HistoryInheritanceStrategy,
		onPromptNeeded?: (card: Card, clozes: any[]) => Promise<InheritanceOptions>
	): Promise<{ card: Card; needsPrompt: boolean }> {
		// 1. 检查内容是否变化
		...
		const wasProgressive = !!(originalCard as any).progressiveCloze;
		const isProgressive = analysis.isProgressiveCloze;
		const hasHistory = !!originalCard.fsrs && originalCard.fsrs.reps > 0;

		logger.debug(
			`[格式检测] 原格式=${wasProgressive ? "渐进式" : "普通"}, 新格式=${
				isProgressive ? "渐进式" : "普通"
			}, 有历史=${hasHistory}`
		);
		...
		if (!wasProgressive && isProgressive) {
			...
			if (hasHistory) {
				...
				return await this.handleHistoryInheritance(
					originalCard,
					updatedCard,
					analysis,
					strategy,
				...
				logger.debug("[场景3-无历史] 转换完成，验证结果:", {
					type: converted.type,
					hasProgressiveCloze: !!(converted as any).progressiveCloze,
	 ...
	 * 处理历史继承
	 */
	private async handleHistoryInheritance(
		originalCard: Card,
		updatedCard: Card,
		analysis: ClozeAnalysisResult,
		strategy: HistoryInheritanceStrategy,
		onPromptNeeded?: (card: Card, clozes: any[]) => Promise<InheritanceOptions>
	): Promise<{ card: Card; needsPrompt: boolean }> {
		...
		const converted = this.convertWithInheritance(
			updatedCard,
			analysis,
			options,
			originalCard.fsrs!
	...
	private updateProgressiveCloze(
		...
		const oldCount = (originalCard as any).progressiveCloze?.totalClozes;
		const newCount = analysis.clozeCount;

		logger.debug(`[挖空数量] 原=${oldCount}, 新=${newCount}`);

		// 保持原有的progressiveCloze结构（V1.5兼容）
		const updatedAny = updatedCard as any;
		const originalAny = originalCard as any;
	 ...
	 * 转换回普通卡片
	 *
	 * Compatibility note: V2架构不需要此方法
	 * V2中父卡片和子卡片是独立的卡片对象，没有"降级"概念
	 * 如果需要删除渐进式挖空，应该删除父卡片和所有子卡片
	 */
	private convertToNormalCard(card: Card): Card {
		...
		cardAny.progressiveCloze = undefined;
		return card;
	}
	 ...
	 * 转换为渐进式挖空格式（无历史继承）
	 */
	private convertToProgressiveCloze(card: Card, analysis: ClozeAnalysisResult): Card {
		const now = new Date().toISOString();

		logger.debug(`[convertToProgressiveCloze] 开始转换，挖空数量: ${analysis.clozeCount}`);

		// ✅ 确保卡片类型为 Progressive（V1.5兼容）
		card.type = "progressive" as any;
	...
	private convertWithInheritance(
		...
		for (let i = 0; i < clozeCount; i++) {
			if (options.mode === "specific" && i === options.targetIndex) {
				...
				cardAny.progressiveCloze.reviewHistory[i] = []; // 清空历史记录
				logger.debug(
					`[按比例] 子挖空 ${i + 1} 获得 ${Math.ceil(parentFsrs.reps / clozeCount)} 次复习`
				);
			} else {
				// 新卡片
				cardAny.progressiveCloze.fsrsData[i] = this.createNewFSRS();
	 ...
	 * 分析卡片内容
	 */
	private analyzeContent(content: string): ClozeAnalysisResult {
		const regex = /\{\{c(\d+)::([^:}]+)(?:::([^}]+))?\}\}/g;
	 ...
	 * 检查序号是否连续
	 */
	private checkSequential(clozes: Array<{ ord: number }>): {
		isSequential: boolean;
		errors?: string[];
	} {
		if (clozes.length === 0) {
	 ...
	 * 创建新的FSRS卡片数据
	 */
	private createNewFSRS(): FSRSCard {
		const now = new Date().toISOString();
