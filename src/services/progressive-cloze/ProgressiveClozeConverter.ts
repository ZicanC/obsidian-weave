...
export class ProgressiveClozeConverter {
  ...
  parseClozes(content: string): ClozeParseResult {
    ...
    if (clozes.length > 0) {
      ...
      if (missingOrds.length > 0) {
        errors.push(
          `Cloze numbers are not continuous. Missing: ${missingOrds.map(o => `c${o + 1}`).join(', ')}`
        );
      }
    }
  ...
  convert(
    ...
    // 创建子卡片
    const children = this.createChildCards(
      sourceCard,
      parseResult.clozes,
      {
        inheritFsrs,
   ...
   */
  private createNewFsrs(daysOffset: number = 0): FSRSCard {
    const now = new Date();
