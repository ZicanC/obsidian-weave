...
export class ContentMigrationTool {
  ...
  async migrate(
    ...
    try {
      ...
      // 2. 扫描子卡片
      const childCardsToMigrate: ProgressiveClozeChildCard[] = [];
      ...
      logger.info(
        `[ContentMigrationTool] 发现 ${result.childCardsCount} 个子卡片，` +
        ...
        for (const childCard of childCardsToMigrate) {
          try {
            ...
            if (!parentCard) {
              ...
              logger.warn(
                `[ContentMigrationTool] 子卡片 ${childCard.uuid} 的父卡片 ${childCard.parentCardId} 不存在`
              );
              continue;
            }
            ...
            if (!parentCard.content) {
              ...
              logger.warn(
                `[ContentMigrationTool] 父卡片 ${parentCard.uuid} 的内容为空`
              );
              continue;
            }
            ...
            logger.debug(
              `[ContentMigrationTool] ✓ 已迁移子卡片: ${childCard.uuid} ` +
            ...
            logger.error(
              `[ContentMigrationTool] ✗ 迁移子卡片失败: ${childCard.uuid}`,
              error
            );
          }
        }
      ...
      if (result.failures.length > 0) {
        ...
        for (const failure of result.failures) {
          logger.warn(
            `  - ${failure.cardId} (父卡片: ${failure.parentCardId}): ${failure.reason}`
          );
        }
      }
 ...
 * 单例实例
 */
let migrationToolInstance: ContentMigrationTool | null = null;

/**
 * 获取迁移工具单例
 */
export function getContentMigrationTool(): ContentMigrationTool {
