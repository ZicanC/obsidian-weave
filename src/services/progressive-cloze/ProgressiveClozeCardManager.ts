...
export class ProgressiveClozeCardManager {
  ...
  async createCard(
    ...
    if (autoSave && this.storageService && result.success) {
      try {
        ...
        logger.info(
          `[ProgressiveClozeCardManager] Saved ${result.cards.length} card(s)`
        );
      } catch (error) {
  ...
  async updateParentCard(parentCard: ProgressiveClozeParentCard): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        `[ProgressiveClozeCardManager] Updated parent card: ${parentCard.uuid}`
      );
  ...
  async deleteProgressiveCloze(parentCardId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        `[ProgressiveClozeCardManager] Deleted progressive cloze: ` +
  ...
  async deleteChildCard(childCardId: string): Promise<boolean> {
    ...
    try {
      ...
      if (updatedChildIds.length < 2) {
        logger.warn(
          `[ProgressiveClozeCardManager] Only ${updatedChildIds.length} child card(s) remaining, ` +
  ...
  getChildCards(parentCardId: string): ProgressiveClozeChildCard[] {
    ...
    return cards.filter((c): c is ProgressiveClozeChildCard => 
