...
const MarkdownHeaderTemplateSystem = {
	parseContentToFields(_content: string): {
		success: boolean;
	...
	generateContentFromFields(
		_fields: Record<string, string>,
		_template: unknown
	): { success: boolean; content?: string; warnings?: string[]; error?: string } {
		return { success: false, error: "MarkdownHeaderTemplateSystem not implemented" };
	},
};
...
export class MDContentParserService {
	...
	private determineSeparationStrategy(
		...
		if (separatorResult.found) {
			const { frontContent, backContent } = this.splitContentBySeparator(
				content,
				separatorResult.pattern!
			);
			return {
				strategy: "separator",
				separatorPattern: separatorResult.pattern,
				frontFields: Object.keys(fields).filter(
					(key) => frontContent.includes(fields[key]) && fields[key].trim()
				),
				backFields: Object.keys(fields).filter(
					(key) => backContent.includes(fields[key]) && fields[key].trim()
				),
	 ...
	 * 按分隔符分割内容
	 */
	private splitContentBySeparator(
		content: string,
		separator: string
	): {
		frontContent: string;
	...
	async parseToFields(content: string, templateId?: string): Promise<MDParseResult> {
		try {
			...
			if (headerParseResult.success && headerParseResult.fields) {
				...
				if (
					templateId &&
					headerParseResult.templateId &&
					headerParseResult.templateId !== templateId
				) {
					logger.warn(
						`内容中的模板ID (${headerParseResult.templateId}) 与指定的模板ID (${templateId}) 不匹配`
					);
				}
	...
	async generateFromFields(
		...
		try {
			...
			if (headerResult.success && headerResult.content) {
				// 应用分隔符优先级逻辑到生成的内容
				const separationStrategy = this.determineSeparationStrategy(headerResult.content, fields);
