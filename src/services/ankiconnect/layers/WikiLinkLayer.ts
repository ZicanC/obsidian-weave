	...
	private convertToObsidianLink(
		...
		const result = content.replace(/\[\[([^\]]+)\]\]/g, (match, _inner, offset) => {
			...
			try {
				...
				// 生成 Obsidian 协议 URL
				const url = UrlEncoder.createObsidianUrl(vaultName, parsed.path, parsed.heading);

				// 显示文本：优先使用自定义文本，否则使用路径
				const displayText =
					parsed.text || (parsed.heading ? `${parsed.path}#${parsed.heading}` : parsed.path);
