...
import { BoundaryDetector, CONVERSION_REGEX } from "../utils/conversion-utils";
...
export class MathFormulaLayer extends BaseConversionLayer {
	 ...
	 * $$...$$ → \[...\]
	 */
	private convertBlockMath(
		content: string,
		_detectCurrency: boolean
	): {
		content: string;
		count: number;
		warnings: string[];
	...
	private isCurrencySymbol(match: string, fullContent: string, offset: number): boolean {
		...
		const afterMatch = fullContent.substring(
			offset + match.length,
			offset + match.length + contextLength
		);
