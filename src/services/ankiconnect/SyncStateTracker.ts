...
export class SyncStateTracker {
	 ...
	 * 初始化或更新卡片同步元数据
	 */
	initializeMetadata(card: Card, ankiNoteId?: number, _isBidirectional = false): CardSyncMetadata {
	 ...
	 * 更新同步元数据（同步成功后调用）
	 */
	updateAfterSync(cardId: string, ankiNoteId?: number, ankiModTime?: string): void {
	...
	detectChanges(card: Card): SyncChange | null {
		...
		return {
			cardId: card.uuid,
			changeType: contentChanged && mediaChanged ? "both" : contentChanged ? "content" : "media",
