...
export class AnkiTemplateConverter {
	...
	convertModelToTemplate(ankiModel: AnkiModelInfo): ConversionResult {
		...
		// 检测卡片类型
		this.detectCardType(ankiModel.fields, ankiModel.templates);
	...
	private detectCardType(
		...
		if (
			fieldNames.some(
				(name) => name.includes("choice") || name.includes("option") || name.includes("选项")
			)
		) {
	...
	isTemplateAlreadyImported(ankiModelId: number): boolean {
		...
		return settings.templates.some(
			(template) => template.syncCapability?.ankiModelMapping?.modelId === ankiModelId
		);
	}
