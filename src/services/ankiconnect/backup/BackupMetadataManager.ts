...
export class BackupMetadataManager {
	...
	async saveIndex(): Promise<void> {
		...
		try {
			...
			await this.plugin.app.vault.adapter.write(indexPath, JSON.stringify(indexData, null, 2));
	...
	async assessDeletability(backupId: string): Promise<DeleteAssessment> {
		...
		const newerBackups = sameDevice.filter(
			(b) =>
	 ...
	 * @returns 设备信息映射
	 */
	private collectDeviceInfo(): Record<string, DeviceInfoSummary> {
		const devices: Record<string, DeviceInfoSummary> = {};
