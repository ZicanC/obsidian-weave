...
export class ObsidianToAnkiConverter {
	...
	constructor(private app: App, private ankiConnect: AnkiConnectClient) {
		// 初始化格式转换器
		this.formatConverter = new ContentFormatConverter();

		// 注册转换层（按优先级顺序）
		this.formatConverter.registerLayer(new MathFormulaLayer()); // 优先级 100
		this.formatConverter.registerLayer(new WikiLinkLayer()); // 优先级 80
		this.formatConverter.registerLayer(new CalloutLayer()); // 优先级 70
		this.formatConverter.registerLayer(new HighlightLayer()); // 优先级 60
		this.formatConverter.registerLayer(new MarkdownLayer()); // 优先级 50 - 基础 Markdown 转换
	}
	...
	async convertContent(
		...
		if (!options.uploadMedia) {
			logger.debug("[ObsidianToAnkiConverter] 媒体同步已关闭，跳过媒体转换与上传");
		} else {
			...
			for (const ref of mediaRefs) {
				...
				if (shouldConvert) {
					...
					if (beforeReplace === result.convertedContent) {
						logger.warn(
							"[ObsidianToAnkiConverter] ⚠️ 替换失败！未找到匹配项:",
							ref.fullMatch.substring(0, 100)
						);
					} else {
	...
	private convertToAnkiFormat(
		...
		switch (type) {
			case "image":
				return alt ? `<img src="${filename}" alt="${alt}">` : `<img src="${filename}">`;
	...
	private generateBacklinks(card: Card, vaultName: string): BacklinkInfo[] {
		...
		// 源文档回链
		if (sourceFile) {
			const sourceLink = this.generateSourceFileBacklink(sourceFile, vaultName);
			if (sourceLink) {
		...
		if (sourceFile && sourceBlock) {
			const blockLink = this.generateBlockBacklink(sourceFile, sourceBlock, vaultName);
			if (blockLink) {
	 ...
	 * 生成源文档回链
	 */
	private generateSourceFileBacklink(sourceFile: string, vaultName: string): BacklinkInfo | null {
		const encodedVault = encodeURIComponent(vaultName);
		const encodedFile = encodeURIComponent(sourceFile);
		const url = `obsidian://open?vault=${encodedVault}&file=${encodedFile}`;
		const html = buildObsidianBacklinkMarkup(url, {
			title: "Open source note in Obsidian",
			marginTop: 12,
		});
