...
export class WeaveCardGenerator {
	 ...
	 */
	generateQACard(fields: Record<string, string>, fieldConfig: FieldSideConfiguration): string {
	...
	convertHTMLtoMarkdown(html: string): string {
		...
		markdown = markdown.replace(/<ol>(.*?)<\/ol>/gis, (_match, content) => {
			...
			return `${items
				.map(
					(item: string, index: number) => `${index + 1}. ` + item.replace(/<\/?li>/gi, "").trim()
				)
				.join("\n")}\n`;
		});
	 ...
	 */
	generatePreview(fields: Record<string, string>, fieldConfig: FieldSideConfiguration): string {
