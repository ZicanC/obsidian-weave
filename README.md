...
- `src/services/editor/` -- 嵌入式编辑器集成（基于 Obsidian 原生编辑器）
...
- `src/utils/licenseManager.ts` -- 许可证验证
- `src/utils/cloudLicenseValidator.ts` -- 云端许可证验证
- `src/utils/security.ts` -- 安全工具
- `src/config/security-config.ts` -- 安全配置
- `src/components/settings/constants/activation-constants.ts` -- 激活常量

其余所有源代码（Svelte 组件、FSRS 算法、数据层、工具函数等）完全开源。

### 付费功能
部分高级功能需要许可证密钥才能使用。以下核心功能完全免费：
- 记忆牌组与 FSRS6 复习调度
- 卡片创建、编辑与管理
- 卡片管理表格视图
- APKG 格式导入
